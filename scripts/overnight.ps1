...
# usage: powershell -ExecutionPolicy Bypass -File scripts\overnight.ps1 [-Seconds 20] [-Bf16]
...
param([int]$Seconds = 20, [switch]$Bf16)
...
$jobs = foreach ($cam in 'C5', 'C3', 'C7') { foreach ($era in '1955', '1980s') { ,@($era, $Seconds, $cam, 'v7') } }
foreach ($j in $jobs) {
  $t0 = Get-Date
