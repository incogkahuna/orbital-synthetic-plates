...
def ensure_traffic():
    ...
    for i in range(30):
        plan.append((f"Traffic_Oncoming{i+1:02d}", -LANE_W,
    ...
    while x < route_len and i < 60:
        if rnd.random() > 0.25:
            plan.append((f"Traffic_Parked{i+1:02d}", LANE_W * 0.95, x, 0.0)); i += 1
...
def sample_route(spline, fps, duration_s, speed_mps):
    ...
    keys = []
    for f in range(n + 1):
        loc, rot = pose_on_route(spline, speed_cm * (f / fps), 0.0, 15.0 + CONFIG["camera_height_cm"])
...
def sample_traffic(spline, traffic, fps, duration_s):
    ...
    for actor, off, start, spd in traffic:
        ...
        for f in frames:
            ...
            loc, rot = pose_on_route(spline, d, off, 15.0 + CAR_H / 2.0, face_backward=(spd < 0))
            keys.append((f, loc, rot))
